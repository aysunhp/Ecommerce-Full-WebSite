    ...
    data.forEach((bag) => {
        ...
        basketArr.forEach((item) => {
            if (bag.id == item.id) {
                shoppingItems.innerHTML += `
                    ...
                    <div class="info">
                      <h1>${bag.name}</h1>
                      <p>
                        Size: XS <span style="margin-left: 15px">Color: ${bag.color}</span>
                      </p>
                    ...
                    <div class="shoppingItem-right">
                      <div class="price">
                        <p style="font-weight:700">US $ <span style="font-weight:700;color:black;font-size:18px">${bag.price - bag.price * bag.discountPercent / 100}</span></p>
                      </div>
                      <div class="buttons">
            ...
            totalPrices.forEach((totalPrice) => {
                let price = Math.round(basketArr.reduce((total, item) => {
                    let addedBag = data.find(bag => bag.id == item.id);
                    return total + (addedBag.price - addedBag.price * addedBag.discountPercent / 100) * item.quantity;
                }, 0));
                console.log(price)
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            totalPrices.forEach((totalPrice) => {
                                ...
                                let removedQuantity = this.parentElement.parentElement.previousElementSibling.children[4].lastChild.innerText;
                                let removedPrice = this.parentElement.parentElement.parentElement.children[2].children[0].children[0].children[0].innerText
                                let price = this.parentElement.parentElement.parentElement.parentElement.nextElementSibling.children[0].children[0].children[2].children[1].children[0].innerText;
...
confirmBtn.addEventListener("click", function (e) {
    e.preventDefault();
    let basketTotal = this.parentElement.previousElementSibling.children[0].children[2].children[1].children[0]
    let localUser = JSON.parse(localStorage.getItem("users"));
    let sessionUser = JSON.parse(sessionStorage.getItem("users"));
    if (localUser || sessionUser) {
